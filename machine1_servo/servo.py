...
if not os.path.isfile(configfile_name):
    ...
    Config.set('data', 'speed','@199:Motor:Speed:M1 1000\r\n') # speed
...
class SERVOStart(tornado.web.RequestHandler):
    def get(self):
        ...
        if sr.is_open:
            ...
            # sr.read(resp)
class SERVOSpeed(tornado.web.RequestHandler):
    def get(self):
        # data = configReader['data']['speed']
        steps = int(self.get_argument("speed"))
        if sr.is_open:
            dt1 = '@199:Motor:Speed:M1 '
            dt2 = str(speed)
            sr.write(dt1+dt2+'\r\n'.encode('ascii'))
            resp = sr.readline()#.decode('utf-8')
            print(resp)
